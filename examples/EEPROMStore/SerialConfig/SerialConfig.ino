/**
 * SerialConfig - シリアルモニタから設定を変更してEEPROMに保存する
 *
 * コマンド一覧（シリアルモニタから送信）:
 *   show          現在の設定を表示
 *   name <value>  デバイス名を変更
...
void printConfig() {
    Serial.println(F("\n=== Device Config ==="));
    Serial.print(F("  name     : ")); Serial.println(config.data.name);
    Serial.print(F("  interval : ")); Serial.print(config.data.intervalMs); Serial.println(F(" ms"));
    Serial.print(F("  offset   : ")); Serial.println(config.data.calibOffset, 3);
    Serial.print(F("  logLevel : ")); Serial.println(config.data.logLevel);
    Serial.println();
}
...
void processCommand(const char* line) {
    if (strncmp(line, "show", 4) == 0) {
        ...
        Serial.print(F("interval -> ")); Serial.println(config.data.intervalMs);
...
void setup() {
    ...
    bool loaded = config.begin();
    Serial.println(loaded ? F("Config loaded from EEPROM") : F("Initialized with defaults"));
    printConfig();
    printHelp();
    Serial.print(F("> "));
}
